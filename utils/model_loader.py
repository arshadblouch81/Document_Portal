...
class ModelLoader:
    ...
    def load_img_reader_llm(self):
        """
        Load and return the configured LLM model.
        """
        llm_block = self.config["image_reader_llm"]
        provider_key = os.getenv("LLM_PROVIDER", "google")
        ...
        temperature = llm_config.get("temperature", 0.2)
        max_tokens = llm_config.get("max_output_tokens", 2048)
        top_p = llm_config.get("top_p", 0.90)
        top_k = llm_config.get("top_k", 40)
        response_mime_type = llm_config.get("response_mime_type", "text/plain")
...
if __name__ == "__main__":
    ...
    # When the script is executed, the `loader` object is created, and it performs the following
    # actions:
    loader = ModelLoader()
    ...
    # llm = loader.load_llm()
    # # Run inference
    # prompt = "What is 8 multiplied by 6?"
